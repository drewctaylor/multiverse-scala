...
import edu.gatech.dt87.multiverse.language.parser
...
object Parser extends TokenParsers with LexerTokens {
  ...
  /**
    ...
    */
  lazy val unit: parser.Parser.Parser[DeclarationStory] = opt(lexical.TokenKeywordStory ~ literalString ~ opt(literalNumber)) ~ opt(stateDeclaration) ~ rep(goalDeclaration) ^^ {
